...
- **Session titles honour the Settings → Appearance language** — The server-side i18n instance has no language-detection plugin (there is no `localStorage` in Bun) and was never re-hydrated from disk, so it always sat at the `en` fallback after restart. Title generation (`generateTitle` / `refreshTitle`) and the system prompt's "Preferred language" line read that server-side language, so they came back in English even when Appearance was set to another language. The chosen UI language is now persisted to an internal, validated `uiLanguage` field and the sidecar i18n is hydrated from it on startup; the renderer also pushes its resolved language to the sidecar once on upgrade so the value is learned without re-picking. Partially addresses [#815](https://github.com/lukilabs/craft-agents-oss/issues/815) and [#738](https://github.com/lukilabs/craft-agents-oss/issues/738). (`4a743ef2`)
