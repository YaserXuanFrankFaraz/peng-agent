...
Use `browser_tool` to drive an agent automation browser (Chromium) inside Craft Agents — a separate browser from the user's built-in browser window (which the user drives manually for browsing and element picking).
...
`console` reads an in-page buffer rather than a live feed. It captures everything from page load onward, except on the page that was already open when `console` was first called on that window — there, only messages after that first call exist. Reload and re-run `console` to get a full page-load history. Results are capped at 100 entries by default (pass a limit to change); when the buffer holds more, the newest entries are kept.
...
The desktop browser manager isn’t wired for this runtime/session. Ensure you’re in the desktop app and the session is initialized.
