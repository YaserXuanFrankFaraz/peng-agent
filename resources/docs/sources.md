...
### 0. Search for Specialized Source Guide (FIRST STEP — when available)

**If the `craft-agents-docs` tools are in your tool list**, search for a specialized
guide before doing anything else:

```
mcp__craft-agents-docs__search_craft_agents({ query: "{service} source setup" })
```
...
**If those tools are NOT in your tool list**, they are unavailable on this backend —
do not call them and do not go looking for them. Skip to step 0.5 and rely on web
search plus the in-app browser to confirm current endpoints, scopes, and auth flow
for the service. The rest of this guide is self-contained.
...
Agent: [FIRST: Searches for Linear guide]
       mcp__craft-agents-docs__search_craft_agents({ query: "linear source setup" })
