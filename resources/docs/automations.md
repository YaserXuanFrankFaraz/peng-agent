...
## Memory And Codex Hooks Compatibility

Craft Agent memory integration reuses the existing AutomationSystem and WorkspaceEventBus flow. It is not a second hook runtime.

Current MVP behavior:

- Memory is controlled by the `codexMemoryEnabled` default and remains off by default.
- `UserPromptSubmit` is the only P0 memory event. Claude and Pi both route it through the same agent event bridge.
- Memory records are stored as user-level JSONL at `~/.craft-agent/memory/memories.jsonl`.
- Memory text is redacted before saving and the file store rejects obvious unredacted secrets.
- Memory context, when supplied by the host, is injected as a bounded `<memory_context>` block below current user/project instructions.
- Codex `hooks.json` compatibility is read-only. Command, prompt, and agent hook handlers are recognized for planning but are not executed.

Codex-native pipeline (FEATURE-002, still off by default via `codexMemoryEnabled`):

- A retrospective two-phase background pipeline runs at workspace init (fire-and-forget, per-workspace cooldown), never blocking sessions.
- Phase 1 extracts redacted memories from idle past sessions into a `bun:sqlite` state DB at `~/.craft-agent/memory/state.db` (Bun process only; unavailable → no-op).
- Phase 2 selects the top-N, renders Markdown products into the git-baselined workspace `~/.craft-agent/memories/`, and — only on a real baseline change — runs a consolidation agent to maintain `MEMORY.md` / `memory_summary.md`.
- The read path injects `memory_summary.md` plus a citable index (`[memory:<id>]`) as a bounded `<memory_context>`; cited ids are scanned back from idle sessions to record usage, which biases future selection.
- Gating, rate caps (`min(maxClaimPerStartup, ratePerMin)`), and retention keep cost and DB growth bounded; logs carry only counts and ids, never memory text.

Still deferred:

- Full Codex hook command execution.
- Hook trust review.
- Managed hooks and plugin-bundled hooks.
- High-frequency `PreToolUse` memory consumption.
- Complete `SessionStart`, `Stop`, `PreCompact`, and `PostCompact` lifecycle equivalence.
